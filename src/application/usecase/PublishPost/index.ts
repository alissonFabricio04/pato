...
export default class PublishPost {
  constructor(readonly postRepository: PostRepository, readonly userRepository: UserRepository) { }

  async handle(input: Input): Promise<Output> {
    const authorExists = await this.userRepository.findById(new Id(input.authorId))
